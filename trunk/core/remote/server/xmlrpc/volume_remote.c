...
xmlrpc_value *get_speaker_volume_RP(xmlrpc_env * const env,
	...
	LOG_DEBUG(VOLUME_REMOTE_MSG_PREFIX "entering getSpeakerVolume...");

	xmlrpc_double result;

	result = get_speaker_volume();
	LOG_DEBUG(VOLUME_REMOTE_MSG_PREFIX "leaving getSpeakerVolume");
...
xmlrpc_value *set_micro_volume_RP(xmlrpc_env * const env,
	...
	LOG_DEBUG(VOLUME_REMOTE_MSG_PREFIX "entering setMicroVolume...");
	...
	LOG_DEBUG(VOLUME_REMOTE_MSG_PREFIX "leaving setMicroVolume");
	return xmlrpc_build_value(env, "b", result);
}
...
xmlrpc_value *get_micro_volume_RP(xmlrpc_env * const env,
	...
	LOG_DEBUG(VOLUME_REMOTE_MSG_PREFIX "entering getMicroVolume...");

	xmlrpc_double result;

	result = get_micro_volume();

	LOG_DEBUG(VOLUME_REMOTE_MSG_PREFIX "leaving getMicroVolume");
	return xmlrpc_build_value(env, "d", result);
}
