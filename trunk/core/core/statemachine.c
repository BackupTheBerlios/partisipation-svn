...
int
sm_initial_state(sm_state * curState, event trigger, void **params) {
    switch (trigger) {
        case GUI_MAKE_CALL:
            *curState = CHECKING;
            break;
...
int
sm_checking_state(sm_state * curState, event trigger, void **params) {
    int rc;
...
int
sm_inviting_state(sm_state * curState, event trigger, void **params) {
    switch (trigger) {
...
int
sm_invited_state(sm_state * curState, event trigger, void **params) {
    switch (trigger) {
        case GUI_END_CALL:
            // sipstack_cancel_call
            *curState = TERMINATING;
...
int
sm_connected_state(sm_state * curState, event trigger, void **params) {
...
int
sm_terminating_state(sm_state * curState, event trigger, void **params) {
...
sm_start(void *args) {
    ...
    while (1) {
        ...
        while (!IsEmpty(eventPool)) {
            ...
            switch (curState) {
                case INITIAL:
                    rc = sm_initial_state(&curState, elem.trigger, elem.params);
                    if (rc != 0) {
                        // ERROR
                    }
                    break;
                ...
                case INVITING:
                    ...
                    break;
                case INVITED:
                    rc = sm_invited_state(&curState, elem.trigger, elem.params);
                    if (rc != 0) {
                        // ERROR
                    }
                    break;
