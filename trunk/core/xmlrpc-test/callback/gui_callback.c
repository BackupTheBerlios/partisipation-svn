...
int
change_reg_status(int accountId, int registered) {
...
int
change_call_status(int callId, char *status) {
...
show_user_event(int accountId,
     ...
     */
    result = xmlrpc_client_call(&callbackEnv, guiURL, methodName,
                                "(issss)", (xmlrpc_int32) accountId, category,
                                title, message, detailMessage);
    die_if_fault_occurred(&callbackEnv);
...
register_core() {
     ...
     * Make the remote procedure call 
     */
    result = xmlrpc_client_call(&callbackEnv, guiURL, methodName, "()", NULL);
    die_if_fault_occurred(&callbackEnv);
...
int
set_speaker_volume_cb(double level) {
    xmlrpc_value *result;
    xmlrpc_bool success;
...
int
set_micro_volume_cb(double level) {
    xmlrpc_value *result;
    xmlrpc_bool success;
