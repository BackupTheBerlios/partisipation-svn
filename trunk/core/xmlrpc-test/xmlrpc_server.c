...
main(int const argc, const char **const argv) {
    ...
    xmlrpc_registry_add_method(&env, registryP, NULL, name, &register_gui_RP,
                               NULL);
    ...
    xmlrpc_registry_add_method(&env, registryP, NULL, name, &end_call_RP, NULL);

    generate_method_name("acceptCall", name);
    xmlrpc_registry_add_method(&env, registryP, NULL, name, &accept_call_RP,
                               NULL);

    // account management:

    generate_method_name("accountGetAll", name);
    xmlrpc_registry_add_method(&env, registryP, NULL, name, &account_get_all_RP,
                               NULL);

    generate_method_name("accountSet", name);
    xmlrpc_registry_add_method(&env, registryP, NULL, name, &account_set_RP,
                               NULL);

    generate_method_name("accountGet", name);
    xmlrpc_registry_add_method(&env, registryP, NULL, name, &account_get_RP,
                               NULL);

    generate_method_name("accountCreate", name);
    xmlrpc_registry_add_method(&env, registryP, NULL, name, &account_create_RP,
                               NULL);

    generate_method_name("accountDelete", name);
    xmlrpc_registry_add_method(&env, registryP, NULL, name, &account_delete_RP,
                               NULL);
